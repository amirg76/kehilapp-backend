...
const globalErrorHandler = (error, req, res, next) => {
  ...
  if (!error.isOperational) {
    logger.error(`unhandled ${statusCode} ${req.method} ${forLog(req.originalUrl)} - ${error.stack || error.message}`);
  } else {
    logger.warn(`${statusCode} ${req.method} ${forLog(req.originalUrl)} - ${error.message}`);
  }
